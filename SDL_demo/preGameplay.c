...
void PreGameplay() {
	...
	window = SDL_CreateWindow("Render Text Animation", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1200, 800, 0);
	renderer = SDL_CreateRenderer(window, -1, SDL_RENDERER_ACCELERATED);

	Mix_Music* music = Mix_LoadMUS("Sound/SoundEffects/typing.mp3");
	...
	font2 = TTF_OpenFont("crazy-pixel.ttf", 80); // font
	...
	font2 = TTF_OpenFont("Kanit-Light.ttf", 20); // font
	int charNumber = 0;
	int lineNumber = 0;

	blackSurface = SDL_LoadBMP("LoginImage/bg.bmp");
	blackTexture = SDL_CreateTextureFromSurface(renderer, blackSurface);

	whitebarSurface = SDL_LoadBMP("LoginImage/whitebar.bmp");
	whitebarTexture = SDL_CreateTextureFromSurface(renderer, whitebarSurface);

	blackbarSurface = SDL_LoadBMP("LoginImage/blackbar.bmp");
